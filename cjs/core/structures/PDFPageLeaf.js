...
var PDFPageLeaf = /** @class */ (function (_super) {
    ...
    PDFPageLeaf.prototype.setColorSpace = function (name, colorSpaceRef) {
        var ColorSpace = this.normalizedEntries().ColorSpace;
        ColorSpace.set(name, colorSpaceRef);
    };
    PDFPageLeaf.prototype.newColorSpaceKey = function (tag) {
        var ColorSpace = this.normalizedEntries().ColorSpace;
        return ColorSpace.uniqueKey(tag);
    };
    PDFPageLeaf.prototype.newColorSpace = function (tag, colorSpaceRef) {
        var ColorSpace = this.normalizedEntries().ColorSpace;
        for (var _i = 0, _a = ColorSpace.entries(); _i < _a.length; _i++) {
            var _b = _a[_i], key = _b[0], value = _b[1];
            if (value === colorSpaceRef)
                return key;
        }
        var newKey = this.newColorSpaceKey(tag);
